...
id: tag.lxb_tag_id_enum_t,

// const Error = error{
//     FailedToInitialize,
// };

pub fn create(id: tag.lxb_tag_id_enum_t) Id {
    return Id{
        .id = id,
    };
}

pub fn tagNameById(self: Id, len: ?*usize) ?*core.lxb_char_t {
    const ret = tag.lxb_tag_name_by_id(@intCast(@intFromEnum(self.id)), len);
    return ret;
}
//
